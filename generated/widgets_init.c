...
static lv_obj_t * zigbee_datetext_date_calendar;
...
void zigbee_datetext_date_event_handler(lv_event_t *e)
{
    ...
    if(code == LV_EVENT_FOCUSED) {
        char * s = lv_label_get_text(btn);
        if(zigbee_datetext_date_calendar == NULL) {
            zigbee_datetext_date_init_calendar(btn, s);
        }
    }
}
...
void zigbee_datetext_date_init_calendar(lv_obj_t *obj, char * s)
{
    if (zigbee_datetext_date_calendar == NULL) {
        lv_obj_add_flag(lv_layer_top(), LV_OBJ_FLAG_CLICKABLE);
        zigbee_datetext_date_calendar = lv_calendar_create(lv_layer_top());
        lv_obj_t * scr = lv_obj_get_screen(obj);
        lv_coord_t scr_height = lv_obj_get_height(scr);
        lv_coord_t scr_width = lv_obj_get_width(scr);
        lv_obj_set_size(zigbee_datetext_date_calendar, scr_width * 0.8, scr_height * 0.8);
        char * year = strtok(s, "/");
        char * month = strtok(NULL, "/");
        char * day = strtok(NULL, "/");
        lv_calendar_set_showed_date(zigbee_datetext_date_calendar, atoi(year), atoi(month));
        lv_calendar_date_t highlighted_days[1];       /*Only its pointer will be saved so should be static*/
        highlighted_days[0].year = atoi(year);
        highlighted_days[0].month = atoi(month);
        highlighted_days[0].day = atoi(day);
        lv_calendar_set_highlighted_dates(zigbee_datetext_date_calendar, highlighted_days, 1);
        lv_obj_align(zigbee_datetext_date_calendar,LV_ALIGN_CENTER, 0, 0);

        lv_obj_add_event_cb(zigbee_datetext_date_calendar, zigbee_datetext_date_calendar_event_handler, LV_EVENT_ALL,NULL);
        lv_calendar_header_arrow_create(zigbee_datetext_date_calendar);
        lv_obj_update_layout(scr);
    }
}
...
void zigbee_datetext_date_calendar_event_handler(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED) {
        ...
        char buf[16];
        lv_snprintf(buf,sizeof(buf),"%d/%02d/%02d", date.year, date.month,date.day);
        lv_label_set_text(guider_ui.zigbee_datetext_date, buf);
        lv_obj_clear_flag(lv_layer_top(), LV_OBJ_FLAG_CLICKABLE);
        lv_obj_set_style_bg_opa(lv_layer_top(), LV_OPA_TRANSP, 0);
        lv_obj_del(zigbee_datetext_date_calendar);
        zigbee_datetext_date_calendar = NULL;
    }
}


extern int zigbee_digital_clock_time_hour_value;
extern int zigbee_digital_clock_time_min_value;
extern int zigbee_digital_clock_time_sec_value;

void zigbee_digital_clock_time_timer(lv_timer_t *timer)
{
    clock_count_24(&zigbee_digital_clock_time_hour_value, &zigbee_digital_clock_time_min_value, &zigbee_digital_clock_time_sec_value);
    if (lv_obj_is_valid(guider_ui.zigbee_digital_clock_time))
    {
        lv_dclock_set_text_fmt(guider_ui.zigbee_digital_clock_time, "%d:%02d:%02d", zigbee_digital_clock_time_hour_value, zigbee_digital_clock_time_min_value, zigbee_digital_clock_time_sec_value);
    }
}
